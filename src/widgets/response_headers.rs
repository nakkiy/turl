...
pub fn draw(f: &mut Frame, area: Rect, app: &mut App) {
    let headers_items: Vec<ListItem> = app
        ...
        .enumerate()
        .map(|(i, (k, v))| {
            let is_selected = app.focus == Focus::ResponseHeaders && i == app.selected_index;
    ...
    let list = List::new(headers_items).block(
        Block::default()
            ...
            .borders(Borders::ALL)
            .title_style(Style::default().fg(if app.focus == Focus::ResponseHeaders {
                Color::Green
            } else {
                Color::default()
            }))
            .border_style(Style::default().fg(if app.focus == Focus::ResponseHeaders {
                Color::Green
            } else {
                Color::DarkGray
            })),
    ...
    f.render_stateful_widget(list, area, &mut app.list_states.response_headers);
}
